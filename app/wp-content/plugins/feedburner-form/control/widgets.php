...
class FeedBurnerFormWidget extends WP_Widget {
    ...
    <div class="fb-container">
        ...
        <div class="fbf-text"><?php echo $instance['filter'] ? wpautop($text) : $text; ?></div>
        <form  id="fbf-widget" action="http://feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open('http://feedburner.google.com/fb/a/mailverify?uri=<?php echo $user; ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520');return true">
        <input type="text"  id="subscription_email" name="email"/><input type="hidden" value="<?php echo $user; ?>" name="uri"/>
...
<p class="fb-credits"><a href="http://wordpress.org/extended/plugin/feedburner-form" target="_blank">FBF</a> &#9642; <a href="http://google.feedburner.com" target="_blank"><?php _e( 'Powered by &reg;Google Feedburner','fbf' ); ?></a></p>
    ...
    <?php if ( $c ) {  ?>
    <div id="fb-counter-img"><a href="http://feeds2.feedburner.com/<?php echo $user; ?>" rel="nofollow"><img src="http://feeds.feedburner.com/~fc/<?php echo  $user.'?bg='.$bgcolor.'&amp;'.'fg='.$ftcolor.'&amp;'.'anim='.$style.'&amp;'.'label='.$textcount; ?>" height="26" width="88" style="border:0" alt="" /></a></div>
    <?php } ?>
    ...
    $instance = wp_parse_args( (array) $instance, array(  'title' => '','text'=>'','user' => 'arquivotk','counter' => 1,'style' => '1','bgcolor'=>'FF9900', 'ftcolor'=> '000','textcount' => 'readers','button'=> 'Ok','credit' => 1 ) );
